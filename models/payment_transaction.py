...
class PaymentTransaction(models.Model):
    ...
    def _compute_capture_eligible(self):
        ...
        for transaction in self:
            transaction.capture_eligible = (
                transaction.provider_code == 'mobilepay' and
                transaction.state == 'authorized' and
                transaction.mobilepay_status == 'RESERVED' and
                transaction.authorized_amount > 0
            )
    ...
    def _send_payment_request(self):
        ...
        try:
            ...
            _logger.error(f"MobilePay payment initiation failed for transaction {self.reference}: {str(e)}")
    ...
    def _mobilepay_get_payment_status(self):
        ...
        try:
            api_client = self.env['mobilepay.api.client']
            status_data = api_client.get_payment_status(self.provider_id, self.mobilepay_payment_id)
            ...
            _logger.error(f"Failed to poll status for transaction {self.reference}: {str(e)}")
            return None
    ...
    def _mobilepay_process_status(self, status_data):
        ...
        if details:
            ...
            if 'capturedAmount' in details:
                vals['captured_amount'] = self._convert_ore_to_dkk(details['capturedAmount'])
            if 'refundedAmount' in details:
                vals['refunded_amount'] = self._convert_ore_to_dkk(details['refundedAmount'])
