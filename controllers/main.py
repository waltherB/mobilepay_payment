...
class MobilePayController(http.Controller):
    ...
    @http.route('/payment/mobilepay/webhook', type='http', auth='public', 
    ...
    def mobilepay_webhook(self, **kwargs):
        ...
        if not auth_header or not content_hash_header or not date_header:
            _logger.warning("MobilePay Webhook: Missing required authentication headers")
        ...
        if not tx_sudo:
            _logger.warning(f"MobilePay Webhook: Transaction not found for paymentId {payment_id}")
            return 'OK'
        ...
        if not self._verify_signature(
            ...
            _logger.warning(f"MobilePay Webhook: Invalid signature for paymentId {payment_id}")
            raise Forbidden("Invalid signature")
            
        # Process Webhook Event
        event_type = data.get('eventType')
        _logger.info(f"MobilePay Webhook: Processing {event_type} for {tx_sudo.reference}")
    ...
    def _verify_signature(self, payload, auth_header, content_hash_header, date_header, host_header, path, secret):
        ...
        try:
            ...
            if not hmac.compare_digest(calculated_hash, content_hash_header):
                _logger.warning(f"MobilePay Webhook: Content hash mismatch. Calc: {calculated_hash}, Header: {content_hash_header}")
                return False
            ...
            # 4. Calculate HMAC
            key = base64.b64decode(secret) # Secret is typically base64 encoded in Vipps response
            
            # If secret wasn't base64, we might simply encode it, but usually these secrets are keys.
            # Let's try to handle both or assume standard. 
            # The doc example shows a secret ending in == so it is base64.
    ...
    @http.route('/payment/mobilepay/return', type='http', auth='public', 
        ...
        if reference:
            ...
            if tx_sudo:
                ...
                try:
                    ...
                    _logger.warning(f"MobilePay Return: Failed to poll status for {reference}: {e}")
