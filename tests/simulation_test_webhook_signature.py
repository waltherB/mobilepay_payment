...
class MockController:
    def _verify_signature(
        ...
        try:
            ...
            # 3. Construct string to sign (v1 format)
            ...
            # 4. Calculate HMAC
            # Test both raw and b64 if the secret looks like a UUID or b64
            # Webhook secret is typically used as a raw UTF-8 string
            ...
            # Fallback: try b64 decoded key if raw fails (for legacy/Azure style)
            try:
                key_b64 = base64.b64decode(secret)
                calculated_hmac_b64 = hmac.new(
                    key_b64, string_to_sign.encode("utf-8"), hashlib.sha256
                ).digest()
                calculated_signature_b64 = base64.b64encode(calculated_hmac_b64).decode(
                    "utf-8"
                )
                return hmac.compare_digest(calculated_signature_b64, provided_signature)
            except Exception:
                pass
...
class TestWebhookAuth(unittest.TestCase):
    def test_azure_signature_verification(self):
        ...
        content_hash = base64.b64encode(hashlib.sha256(payload).digest()).decode(
            "utf-8"
        )
        ...
        # 3. Calculate Signature using the secret (Raw UTF-8)
        key = secret.encode("utf-8")
        ...
        result = controller._verify_signature(
            payload, auth_header, content_hash, date, host, path, secret
        )
        ...
        # Test Tampering
        tampered_payload = json.dumps({"data": {"paymentId": "666"}}).encode(
            "utf-8"
        )  # Evil payload
        ...
        self.assertFalse(
            result_tampered, "Tampered payload content hash check should fail"
        )
